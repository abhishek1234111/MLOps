...
model_uri = "models:/AmesHousingModel/Production"
...
def detect_model_drift(predictions_log, actuals_log, threshold=0.05):
    ...
    mse = mean_squared_error(data['Actual'], data['Prediction'])
    
    # Load initial model performance
    initial_mse = pd.read_csv('initial_model_performance.csv').iloc[0]['mse']
    
    # Calculate drift
    drift = abs(initial_mse - mse) / initial_mse
    return drift, mse
